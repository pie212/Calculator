...
if st.session_state.status == 0:      # 0 = home, 1-4 math, 5 chem
    ...
    if(st.button('Submit')): 
        try:
            a,b,c = cleaned.FindTerms(func1)
            try:
                alfa,beta = cleaned.AlphaBetaForm(func1)
                d,x1,x2 = cleaned.Solution(func1)
                if a == 0:
    ...
    if(st.button('Submit')):
        ...
        a,b,c = cleaned.FindTerms(result)
        f = lambda x: float(a)*x**2 + float(b)*x + float(c)
    ...
    if(t):
        ...
        try:
            ...
            a,b,c = cleaned.FindTerms(func1)
            a1,b1,c1 = cleaned.FindTerms(func2)
    ...
    if(st.button('Submit')):
        ...
        for i in range(len(funcsL)):
            ...
            a,b,c = cleaned.FindTerms(funcsL[i])
            f = lambda x: float(a)*x**2 + float(b)*x + float(c)
            y = f(x)
